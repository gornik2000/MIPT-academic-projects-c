//-----------------------------------------------------------------------------
///
/// Words that can be used:
///   x  y  z
///   pi  e  phi  ls
///   +  -  /  *  ^
///   sin  cos  tan  cot
///   arcsin  arccos  arctan  arccot
///   ln  ()
///
//-----------------------------------------------------------------------------
/* if you need optimization, add to optimization node function */
/* name - value - type - priority - arg number - derivative function*/
//-----------------------------------------------------------------------------
//#define D(n)         nodeCreateDerivative (n)
//#define C(n)         nodeCreateCopy (n)
//#define NC(k, l, r)  nodeCreate (k, l, r)
//#define DC(t, p, v)  ddataCreate(t, p, v)
//-----------------------------------------------------------------------------
/* variables  */
/* сделать список функций и переменных */
//-----------------------------------------------------------------------------
/* operators */

DEF_DIFF ("=",        1,  OP, 10, 2,
          {
          })

DEF_DIFF ("if",       2,  OP, 10, 2,
          {
          })

DEF_DIFF ("else",     3,  OP, 10, 1,
          {
          })

DEF_DIFF ("elif",     4,  OP, 10, 2,
          {
          })

DEF_DIFF ("while",    5,  OP, 10, 2,
          {
          })

DEF_DIFF ("for",      6,  OP, 10, 2,
          {
          })

DEF_DIFF ("break",    7,  OP, 10, 0,
          {
          })

DEF_DIFF ("continue", 8,  OP, 10, 0,
          {
          })

DEF_DIFF ("def",      9,  OP, 10, 2,
          {
          })
DEF_DIFF ("return",   10, OP, 10, 1,
          {
          })

DEF_DIFF ("<",        11, OP, 10, 2,
          {
          })

DEF_DIFF (">",        12, OP, 10, 2,
          {
          })

DEF_DIFF ("==",       13, OP, 10, 2,
          {
          })

DEF_DIFF ("<=",       14, OP, 10, 2,
          {
          })
DEF_DIFF (">=",       15, OP, 10, 2,
          {
          })

DEF_DIFF ("and",      16, OP, 10, 2,
          {
          })

DEF_DIFF ("or",       17, OP, 10, 2,
          {
          })

DEF_DIFF ("+",        30, OP, 10, 2,
          {
          })

DEF_DIFF ("-",        31, OP, 10, 2,
          {
          })

DEF_DIFF ("++",       32, OP, 10, 1,
          {
          })

DEF_DIFF ("--",       33, OP, 10, 1,
          {
          })

DEF_DIFF ("*",        34, OP, 10, 2,
          {
          })

DEF_DIFF ("/",        35, OP, 10, 2,
          {
          })

DEF_DIFF ("%",        36, OP, 10, 2,
          {
          })

DEF_DIFF ("//",       37, OP, 10, 2,
          {
          })

DEF_DIFF ("ln",       38, OP, 10, 1,
          {
          })

DEF_DIFF ("sin",      39, OP, 10, 1,
          {
          })

DEF_DIFF ("cos",      40, OP, 10, 1,
          {
          })

DEF_DIFF ("tan",      41, OP, 10, 1,
          {
          })

//-----------------------------------------------------------------------------
/* constants  */
DEF_DIFF ("pi",  3.141592654, CNST, 255, 0, ;)
DEF_DIFF ("e",   2.718281828, CNST, 255, 0, ;)
DEF_DIFF ("phi", 1.618033989, CNST, 255, 0, ;)
DEF_DIFF ("lc",  299792458,   CNST, 255, 0, ;)
//-----------------------------------------------------------------------------
//#undef D
//#undef NCC
//#undef NC
//#undef DC
//-----------------------------------------------------------------------------
