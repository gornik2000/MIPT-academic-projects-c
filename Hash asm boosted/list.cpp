...
char listIsStrInList (list *l, list_value value);
...
list_data *listFindNum   (list *l, int num)
{
  ...
  if (num > 0) for (int i = 0; i < num; i++) elem = elem->next;
  if (num < 0) for (int i = num; i < 0; i++) elem = elem->prev;
...
char listIsStrInList (list *l, list_value value)
{
  ...
  for (int i = 0; i < l->count; i++)
  {
    if (strcmp(*(elem->value), value) == 0) return 1;
    elem = elem->next;
  }
  return 0;
}
...
char listEmplaceBack  (list *l, list_value value, int num)
{
  ...
  *(elem->value) = value;
  ...
  if (elem->prev == l->tail) l->tail = elem;
...
char listPushFront (list *l, list_value value)
{
  ...
  if (l->count == 0) return listPushFirst (l, value);
...
char listRemove (list *l, int num)
{
  ...
  if (l->count <= 0) return ERR_NEG_COUNT;
  if (l->count == 1) return listErase (l);

  list_data *elem = listFindNum (l, num);
  (elem->next)->prev = (elem->prev);
  (elem->prev)->next = (elem->next);

  if (elem == l->head) l->head = elem->next;
  if (elem == l->tail) l->tail = elem->prev;
...
char listVerification (list *l)
{
  ...
  for (int i = 1; i < l->count; i++) elem = elem->next;
  if (elem != l->tail) return ERR_PTR;

  for (int i = 1; i < l->count; i++) elem = elem->prev;
